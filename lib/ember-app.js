...
function serializeResponse(doc, rootElement) {
  return function(instance) {
    ...
    if (doc.head) {
      contentForHead += HTMLSerializer.serializeChildren(doc.head);
    }
    if (doc.title) {
      contentForHead += '<title>' + doc.title + '</title>';
    }

    var contentForBody = HTMLSerializer.serializeChildren(rootElement);

    // TODO extract statusCode, headers, attributesForHtml and attributesForBody from instance
    // var FastBootResponse = instance.lookup('info:-fastboot').get('response');
    // statusCode = FastBootResponse.get('statusCode');
    // headers = FastBootResponse.get('headers');
    // attributesForHtml = FastBootResponse.attributesFor('html');
    // attributesForBody = FastBootResponse.attributesFor('body');
