...
FastBootServer.prototype.insertIntoIndexHTML = function(contentHash, attributesHash) {
  var html = this.html;

  Object.keys(contentHash).forEach(function (key) {
    html = html.replace('<!-- FASTBOOT_CONTENT_FOR_' + key.toUpper() + ' -->', contentHash[key]);
  });

  Object.keys(attributesHash).forEach(function (key) {
    html = html.replace(' FASTBOOT-ATTRIBUTES-FOR-' + key.toUpper() + '', attributesHash[key]);
  });
...
FastBootServer.prototype.handleSuccess = function(res, path, result, startTime) {
  ...
  res.send(this.insertIntoIndexHTML(result.contentHash, result.attributesHash));
};
